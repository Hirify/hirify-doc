...
La page **Sécurité** réunit deux choses : votre mot de passe et vos jetons d'accès. Elle s'ouvre depuis le menu à votre nom, en bas de la barre latérale, puis **Paramètres** et **Sécurité**. L'authentification unique se met en place avec l'équipe Hirify.
...
Une fois créé, le jeton apparaît dans la liste avec sa date de création. Sa date de dernière utilisation s'affiche dès qu'il a servi. C'est le moyen le plus simple de repérer un jeton devenu inutile.
...
Sa mise en place ne passe pas par l'application : il n'existe pas d'écran client pour l'activer ou la configurer. Elle se fait avec l'équipe Hirify, qui enregistre votre domaine et les métadonnées de votre fournisseur d'identité. Elle relève de l'offre Volume.
